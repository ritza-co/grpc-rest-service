...
import (
	...
	"github.com/speakeasy-api/grpc-rest-service/fixtures"
...
func (s *barServer) CreateDrink(ctx context.Context, req *bar.CreateDrinkRequest) (*bar.DrinkResponse, error) {
	...
	var name, description, recipe, photo string
	...
	if (openaiAPIKey != "") {
		...
		// Use the fixtures data if the OpenAI API key is not set
		name = fixtures.Name
		description = fixtures.Description
		recipe = fixtures.Recipe
...
func (s *barServer) GetDrink(ctx context.Context, req *bar.GetDrinkRequest) (*bar.DrinkRecipeResponse, error) {
	...
	var recipe, photo string
	var ingredients []*bar.IngredientQuantity
	...
	if (openaiAPIKey != "") {
		...
		// Use the fixtures data if the OpenAI API key is not set
		ingredients = make([]*bar.IngredientQuantity, len(fixtures.Ingredients))
		for i, ingredient := range fixtures.Ingredients {
			ingredients[i] = &bar.IngredientQuantity{
...
func main() {
	...
	if openaiAPIKey == "" {
		log.Printf("Note: You can set the OPENAI_API_KEY environment variable to use the OpenAI API.")
	}
